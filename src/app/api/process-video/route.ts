...
const openai = new OpenAI({
	apiKey: "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",
});
...
interface StreamWriter {
	write: (data: { status?: { step: number; message: string }; transcript?: string; summary?: string }) => void;
	close: () => void;
	error: (error: Error) => void;
}

// Extract audio from YouTube video
async function extractAudio(url: string, writer: StreamWriter): Promise<Buffer> {
	writer.write({
	...
	try {
		...
		const audioBuffer = await new Promise<Buffer>((resolve, reject) => {
			audioStream.on('end', () => {
				...
				writer.write({
					status: { step: 1, message: `Audio download completed (${finalSizeMB}MB)` }
				});
				resolve(finalBuffer);
			});
...
async function compressAudio(inputBuffer: Buffer, writer: StreamWriter): Promise<Buffer> {
	const originalSizeMB = (inputBuffer.length / (1024 * 1024)).toFixed(2);
	writer.write({
		status: { step: 2, message: `Compressing ${originalSizeMB}MB audio file...` }
	});
			...
			.then(() => {
				...
				ffmpeg(inputPath)
					...
					.on('progress', (progress) => {
						if (progress.percent) {
							writer.write({
								status: { step: 2, message: `Compressing audio... ${Math.round(progress.percent)}%` }
							});
						}
					})
					.on('end', async () => {
						try {
							...
							writer.write({
								status: { step: 2, message: `Audio compressed: ${originalSizeMB}MB → ${compressedSizeMB}MB` }
							});
...
async function streamTranscribeAudio(audioBuffer: Buffer, writer: StreamWriter): Promise<string> {
	const totalSizeMB = (audioBuffer.length / (1024 * 1024)).toFixed(1);

	writer.write({
		status: { step: 3, message: `🚀 STREAMING MODE: Processing ${totalSizeMB}MB audio with real-time transcription` }
	});
	...
	try {
		...
		writer.write({
			status: { step: 3, message: `⏱️ Audio duration: ${Math.floor(duration / 60)}m ${Math.round(duration % 60)}s. Creating ${totalChunks} streaming chunks...` }
		});

		const transcriptResults: { index: number; text: string; startTime: number; endTime: number }[] = [];
		...
		for (let i = 0; i < totalChunks; i++) {
			...
			writer.write({
				status: { step: 3, message: `🎵 Creating chunk ${i + 1}/${totalChunks} (${startTime}s-${Math.round(endTime)}s)...` }
			});
			...
			try {
				...
				writer.write({
					status: { step: 3, message: `📤 Transcribing chunk ${i + 1}/${totalChunks}...` }
				});

				// Read the chunk and transcribe it
				const chunkBuffer = await fs.readFile(chunkPath);
				const audioFile = new File([chunkBuffer], `chunk_${i}.wav`, { type: 'audio/wav' });
				...
				writer.write({
					status: { step: 3, message: `✅ Chunk ${i + 1}/${totalChunks} completed (${chunkWords} words) - "${transcription.text.substring(0, 50)}${transcription.text.length > 50 ? '...' : ''}"` }
				});

				// Stream partial transcript immediately in correct order
				if (transcription.text.trim()) {
					cumulativeTranscript += (cumulativeTranscript ? ' ' : '') + transcription.text;
		...
		writer.write({
			status: { step: 3, message: `🎉 Streaming transcription completed! Total: ${totalWords} words from ${totalChunks} chunks` }
		});
		...
		throw new Error(`Failed to stream transcribe audio: ${error instanceof Error ? error.message : 'Unknown error'}`);
	}
}
...
async function transcribeAudio(audioBuffer: Buffer, writer: StreamWriter): Promise<string> {
	const audioSizeMB = (audioBuffer.length / (1024 * 1024)).toFixed(1);
	const maxSizeBytes = 25 * 1024 * 1024; // 25MB

	writer.write({
		status: { step: 3, message: `Starting transcription of ${audioSizeMB}MB audio file...` }
	});
	...
	if (estimatedDuration > 60) { // For videos longer than 1 minute, use streaming
		writer.write({
			status: { step: 3, message: `🚀 Video duration ~${Math.floor(estimatedDuration / 60)}m ${estimatedDuration % 60}s - using streaming transcription for faster results` }
		});
		return await streamTranscribeAudio(audioBuffer, writer);
	} else if (audioBuffer.length <= maxSizeBytes) {
		writer.write({
			status: { step: 3, message: `✅ Short video (${audioSizeMB}MB) - using direct transcription` }
		});
	} else {
		writer.write({
			status: { step: 3, message: `⚠️ Audio file (${audioSizeMB}MB) exceeds 25MB limit - will try compression first, then streaming if needed` }
		});
	}
	...
	try {
		...
		if (audioBuffer.length > maxSizeBytes) {
			writer.write({
				status: { step: 2, message: `🗜️ Attempting compression to avoid chunking...` }
			});
			...
			try {
				...
				if (processedBuffer.length > maxSizeBytes) {
					const compressedSizeMB = (processedBuffer.length / (1024 * 1024)).toFixed(1);
					writer.write({
						status: { step: 3, message: `❌ Compression insufficient (${compressedSizeMB}MB still > 25MB) - switching to streaming method` }
					});
					return await streamTranscribeAudio(audioBuffer, writer);
				} else {
					const compressedSizeMB = (processedBuffer.length / (1024 * 1024)).toFixed(1);
					writer.write({
						status: { step: 3, message: `✅ Compression successful (${compressedSizeMB}MB) - proceeding with direct transcription` }
					});
				}
			} catch {
				writer.write({
					status: { step: 3, message: `❌ Audio compression failed - switching to streaming method` }
				});
				return await streamTranscribeAudio(audioBuffer, writer);
			}
		}
		...
		writer.write({
			status: { step: 3, message: '📤 Sending audio to OpenAI Whisper for transcription...' }
		});

		// Create a File-like object from the buffer
		const audioFile = new File([processedBuffer], 'audio.ogg', { type: 'audio/ogg' });

		writer.write({
			status: { step: 3, message: '🤖 Whisper is processing the audio (this may take a few minutes)...' }
		});
		...
		writer.write({
			status: { step: 3, message: `✅ Direct transcription completed! Generated ${wordCount} words (${transcriptLength} characters)` }
		});
		...
		if (error instanceof Error && error.message.includes('413')) {
			writer.write({
				status: { step: 3, message: '❌ File still too large for Whisper API - switching to streaming method as fallback' }
			});
			return await streamTranscribeAudio(audioBuffer, writer);
		}
...
async function translateTextInChunks(text: string, targetLanguage: 'en' | 'fr', writer: StreamWriter): Promise<string> {
	...
	writer.write({
		status: { step: 4, message: `Translating ${chunks.length} text segments to ${languageName}...` }
	});
	...
	for (let i = 0; i < chunks.length; i++) {
		writer.write({
			status: { step: 4, message: `Translating segment ${i + 1}/${chunks.length} to ${languageName}...` }
		});
...
async function translateText(text: string, targetLanguage: 'en' | 'fr', writer: StreamWriter): Promise<string> {
	...
	try {
		...
		const translation = response.choices[0]?.message?.content || 'Failed to translate';
...
async function summarizeTextInChunks(text: string, writer: StreamWriter, language: Language): Promise<string> {
	...
	writer.write({
		status: { step: 5, message: `Creating summaries for ${chunks.length} text segments...` }
	});
	...
	for (let i = 0; i < chunks.length; i++) {
		...
		try {
			...
			const summary = response.choices[0]?.message?.content || `Segment ${i + 1} summary unavailable`;
	...
	try {
		...
		const finalSummary = response.choices[0]?.message?.content || consolidatedText;
...
async function summarizeText(text: string, writer: StreamWriter, language: Language): Promise<string> {
	// Check if text is too long for single request
	if (text.length > 3000) {
...
async function translateIfNeeded(text: string, recapLanguage: Language, writer: StreamWriter): Promise<string> {
	const textLength = text.length;
	const wordCount = text.split(' ').length;

	writer.write({
		status: { step: 4, message: `Analyzing transcript language (${wordCount} words, ${textLength} characters)...` }
	});
	...
	const frenchIndicators = /\b(le|la|les|de|du|des|et|est|une|un|dans|pour|avec|sur|par|ce|cette|qui|que|mais|ou|où|donc|car|si|comme|tout|tous|toute|toutes|très|plus|moins|bien|encore|aussi|déjà|jamais|toujours|peut|peuvent|faire|avoir|être|aller|venir|voir|savoir|dire|prendre|donner|partir|sortir|entrer|monter|descendre)\b/gi;
	...
	writer.write({
		status: { step: 4, message: `Detected language: ${detectedLanguage}. Target recap language: ${targetLanguage}` }
	});

	// If transcript appears to be in French and user wants English recap, translate
	if (isFrench && recapLanguage === 'en') {
		writer.write({
			status: { step: 4, message: 'Translation needed: French → English. Starting translation...' }
		});
	...
	if (!isFrench && recapLanguage === 'fr') {
		writer.write({
			status: { step: 4, message: 'Translation needed: English → French. Starting translation...' }
		});
	...
	writer.write({
		status: { step: 4, message: `No translation needed - transcript and recap are both in ${targetLanguage}` }
	});
...
export async function POST(request: NextRequest) {
	...
	try {
		const { url, recapLanguage = 'en' } = await request.json();
		...
		const stream = new ReadableStream({
			async start(controller) {
				const writer: StreamWriter = {
					write: (data: { status?: { step: number; message: string }; transcript?: string; summary?: string }) => {
				...
				try {
					...
					const audioBuffer = await extractAudio(url, writer);
					...
					transcript = await translateIfNeeded(transcript, recapLanguage, writer);

					writer.write({ transcript });

					// Step 5: Summarize transcript in the desired language
					const summary = await summarizeText(transcript, writer, recapLanguage);
					writer.write({ summary });

					writer.close();
				} catch (error) {
					console.error('Processing error:', error);
					writer.error(error instanceof Error ? error : new Error('Unknown error occurred'));
				}
			},
		});
